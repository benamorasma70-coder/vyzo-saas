...
import { SHARED_STYLES } from '../../shared-styles'   // ← chemin adapté si centralisé
    ...
    <>
      ...
      <div className="root">
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          >
            <ArrowLeft size={16} /> Retour aux bons de livraison
          </button>
          ...
          <div className={`glass fade-up ${loaded ? 'show' : ''}`} style={{ padding: 24 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div>
                <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700 }}>
                  BL N° {delivery.delivery_number}
                </h1>
                <p style={{ color: 'var(--muted)' }}>
                  Date de livraison : {new Date(delivery.delivery_date).toLocaleDateString('fr-FR')}
                </p>
              </div>
              <select
            ...
            {delivery.notes && (
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20, marginTop: 20 }}>
                ...
                <p style={{ color: 'var(--muted)' }}>{delivery.notes}</p>
              </div>
            )}
          </div>
        </div>
